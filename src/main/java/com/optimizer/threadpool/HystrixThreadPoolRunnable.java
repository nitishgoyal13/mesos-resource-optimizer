...
public class HystrixThreadPoolRunnable implements Runnable {
    ...
    @Builder
    public HystrixThreadPoolRunnable(GrafanaService grafanaService, ThreadPoolConfig threadPoolConfig,
    ...
    private void handleHystrixPool(String serviceName, Map<String, List<String>> serviceVsPoolList, String ownerEmail,
        ...
        Map<String, Long> hystrixPoolVsMaxPool = executePoolQuery(hystrixPools, MAX_POOL_QUERY,
                ExtractionStrategy.AVERAGE, cluster);
        Map<String, Long> hystrixPoolVsPoolsUsage = executePoolQuery(hystrixPools, POOL_USAGE_QUERY,
                ExtractionStrategy.MAX, cluster);

        if (CollectionUtils.isEmpty(hystrixPoolVsPoolsUsage) || CollectionUtils.isEmpty(hystrixPoolVsMaxPool)) {
            LOGGER.error(
                    "Error in getting hystrix pools list for Service: {}. Got poolsUsage = {}. Got poolsCore = {} ",
                    serviceName,
        ...
        for (String hystrixPool : hystrixPools) {
            ...
            reduceBy = reducePoolUsage(serviceName, ownerEmail, maxPool, poolUsage, hystrixPool, reduceBy,
                    usagePercentage);
            extendBy = extendPoolUsage(serviceName, ownerEmail, maxPool, poolUsage, hystrixPool, extendBy,
                    usagePercentage);
            LOGGER.info("Service: {} Type: HYSTRIX Pool: {} Max: {} Usage: {} Reduce: {} Extend: {}", serviceName,
                    hystrixPool, maxPool,
    ...
    private long extendPoolUsage(String serviceName, String ownerEmail, long maxPool, long poolUsage,
            String hystrixPool, long extendBy,
        ...
        if (usagePercentage > threadPoolConfig.getThresholdMaxUsagePercentage()) {
            ...
            if (extendBy > threadPoolConfig.getExtendByThreshold()) {
                mailSender.send(MAIL_SUBJECT,
                        getExtendByMailBody(serviceName, hystrixPool, maxPool, poolUsage, extendBy, ownerEmail,
    ...
    private long reducePoolUsage(String serviceName, String ownerEmail, long maxPool, long poolUsage,
            String hystrixPool, long reduceBy,
            long usagePercentage) {
        if (usagePercentage < threadPoolConfig.getThresholdMinUsagePercentage()) {
            ...
            if (reduceBy > threadPoolConfig.getReduceByThreshold()) {
                mailSender.send(MAIL_SUBJECT,
                        getReduceByMailBody(serviceName, hystrixPool, maxPool, poolUsage, reduceBy, ownerEmail,
    ...
    private Map<String, Long> executePoolQuery(List<String> hystrixPools, String query,
    ...
    private String getReduceByMailBody(String serviceName, String pool, long maxPool, long poolUsage, long reduceBy,
            String ownerEmail,
    ...
    private String getExtendByMailBody(String serviceName, String pool, long maxPool, long poolUsage, long extendBy,
            String ownerEmail,
            int threshodMaxUsagePercentage) {
        return String
                .format("Hi, %s <br> Hystrix Thread Pool can be optimized. Thread pool usage has gone above %s%% in last 8 days. "
                                +
                        ...
                        ownerEmail, Integer.toString(threshodMaxUsagePercentage), serviceName, pool, maxPool, poolUsage,
                        extendBy
                );
    }
