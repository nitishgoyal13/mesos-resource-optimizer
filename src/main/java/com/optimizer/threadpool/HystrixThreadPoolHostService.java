...
public class HystrixThreadPoolHostService implements Runnable {
    ...
    private void handleHystrixPoolByHost(String cluster, String serviceName, Map<String, List<String>> serviceVsPoolList,
        ...
        for(String hystrixPool : CollectionUtils.nullSafeList(hystrixPools)) {
            ...
            Map<String, Integer> hostVsMaxPool = executePoolQueryByHost(cluster, hystrixPool, MAX_POOL_QUERY_BY_HOST,
                                                                        ExtractionStrategy.AVERAGE
                                                                       );
            Map<String, Integer> hostVsPoolUsage = executePoolQueryByHost(cluster, hystrixPool, POOL_USAGE_QUERY_BY_HOST,
                                                                          ExtractionStrategy.MAX
                                                                         );

            for(Map.Entry<String, Integer> entry : CollectionUtils.nullSafeMap(hostVsMaxPool)
                    .entrySet()) {
                ...
                int maxPool = hostVsMaxPool.get(host);
                int poolUsage = hostVsPoolUsage.get(host);
                ...
                if(usagePercentage < threadPoolConfig.getThresholdMinUsagePercentage()) {
                    int reduceBy = ((maxPool * threadPoolConfig.getThresholdMinUsagePercentage()) / 100) - poolUsage;
                    if(reduceBy > threadPoolConfig.getReduceByThreshold()) {
                        optimisedConfig.setOptimisedPoolValue(maxPool - reduceBy);
                ...
                if(usagePercentage > threadPoolConfig.getThresholdMaxUsagePercentage()) {
                    int extendBy = poolUsage - ((maxPool * threadPoolConfig.getThresholdMinUsagePercentage()) / 100);
                    if(extendBy > 0) {
                        optimisedConfig.setOptimisedPoolValue(maxPool + extendBy);
    ...
    private Map<String, Integer> executePoolQueryByHost(String cluster, String hystrixPool, String query,
    ...
    private Map<String, Integer> executeGrafanaQueriesByHost(String query, ExtractionStrategy extractionStrategy) throws Exception {
        Map<String, Integer> hostVsResult = new HashMap<>();
        ...
        if(jsonObject.has(RESULTS)) {
            ...
            for(int index = 0; index < seriesJSONArray.length(); index++) {
                ...
                int value = getValueFromGrafanaResponseByHost(seriesObject.toString(), extractionStrategy);
