...
public class ThreadPoolQueryUtils {
    ...
    public static final String CORE_POOL_QUERY =
            "SELECT sum(\"value\") FROM \"%s.%s.propertyValue_corePoolSize\" WHERE time > now() - %sh " + "group by time(30s) fill(null)";

    public static final String MAX_POOL_QUERY =
            "SELECT sum(\"value\") FROM \"%s.%s.propertyValue_actualMaximumSize\" WHERE time > now() - %sh " +
